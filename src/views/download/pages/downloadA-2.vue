<template>
  ...
  <div class="warp">
    <div class="banner">
      <div class="nav"><span class="green" @click="router.push('/')">{{ $t('message.download.nav_txt1') }}</span> >
        {{ $t('message.download.nav_txt2') }}
      </div>
      <div class="banner_txt">
        <div class="welmsg">{{ $t('message.download.welmsg_txt1') }} <span>{{
            $t('message.download.welmsg_txt2')
          }}</span></div>
        <div class="title">{{ $t('message.download.title') }}</div>
      </div>
    </div>
    <div class="register">
      <div class="step1"><span class="green">{{ $t('message.download.step1_name') }} :</span>
        {{ $t('message.download.welmsg_txt2') }}
      </div>
      <div class="step1_content">
        ...
        <form action="#" onsubmit="">
          ...
          <div class="button">
            <input type="text" v-model="emailAddress" @input="emailInput()"
                   :placeholder="$t('message.download.inputEmail')"/>
    ...
    <div class="down_button">
      ...
      <div class="step2_wrap">
        ...
        <div class="step2">
          <span class="green">{{ $t('message.download.step2_name') }} :</span>
          <br/>{{ $t('message.download.download_txt') }} <br/>
      ...
      <div class="buttons flex_between_center">
        <div class="down_cyberpop" @mouseenter="AndroidQcode = true" @mouseleave="AndroidQcode = false"
             @click="downloadGame(0)">
...
const downloadGame = (type: number) => {
  ...
  switch (type) {
    ...
    case 0:
      ...
      break;
    case 1:
      window.location.href = 'https://zengeon-cos-assets-1259788321.cos.ap-shanghai.myqcloud.com/PrinceOfPersia/Jenkins/AppFiles/Cyberpop_0.0.8_2022_07_07_21_44_49_V25_Dev.false_Symbols.USE_NFT_ASSETS.exe'
  }
...
const messgSing = async (publicAddress: any) => {
  try {
    ...
    if (result) {
      ...
      store.dispatch('user/showDialog', {
        show: true,
        info: { state: 0, txt: t('message.assets.pop.reject_transaction') }
      })
    }
  } catch (err) {
